...
#g2_x = np.array(np.exp(tmp/13-4)-(tmp-15)/15)
g2_x = np.array(np.exp(tmp/20)-tmp/10-0.5)
#pi = 1/(1+15*np.exp(g2_x[:,0]-1.7*y[:,1]))
pi = 1/(1+10*np.exp(g2_x[:,0]))
...
for epoch in range(1, NB_EPOCH+1):
    ...
    # Single training iteration (we mask nodes without labels for loss calculation)
    model.fit(graph, y_train, sample_weight=sam_weight,
              batch_size=A.shape[0], epochs=1, shuffle=False, verbose=0)
