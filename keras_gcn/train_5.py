...
g2 = np.array(np.exp(tmp/13-4)-(tmp-15)/20)
pi = 1/(1+20*np.exp(g2[:,0]-1.5*y[:,1]))
...
tmp = np.ones((y.shape))
tmp[:,1] = np.random.binomial(size=preds_y.shape[0], n=1, p=preds_y[:,1])

for i in range(y.shape[0]):
    p = np.min([preds_y[i,int(tmp[i,1])]/preds_old[i,int(y_est[i,1])],1])
    select = np.random.binomial(size=1, n=1, p=p)
    if select == 0:
        tmp[i,1] = y_est[i,1]

tmp[idx_train+idx_val,1] = y[idx_train+idx_val,1]
y_est = tmp
